...
namespace Smile
{
	namespace Raster
	{
		void FineRasterizer(Bin* pBins, uint32_t binCountX, uint32_t binCountY, uint32_t binWidth, uint32_t binHeight, VS_OUTPUT* pPixelData, float* pDepthBuffer, uint32_t width)
		{
		...
		__global__ void FineRasterizerKernel(Segment* pBins, const Triangle* pTriangles, uint32_t binCountX, uint32_t binCountY, uint32_t binWidth, uint32_t binHeight, VS_OUTPUT* pPixelData, uint32_t* pPixelLock, float* pDepthBuffer, uint32_t width)
		{
				...
				for (uint32_t t{}; t < bin.QueueSize; ++t)
				{
					...
					for (uint32_t y{ minY }; y < maxY; ++y)
					{
						for (uint32_t x{ minX }; x < maxX; ++x)
						{
							...
							/*pPixelData[pixelIndex].Position = { pixel.x, pixel.y, -0.5f, 0.f };
							pPixelData[pixelIndex].Color = { 1.0f, 0.f, 0.f };*/
