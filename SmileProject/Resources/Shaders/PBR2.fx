...
float TrowbridgeReitzGGX(float3 normal, float3 halfVector, float roughness)
{
    ...
    float a2 = roughnessSqr * roughnessSqr;
    float NdotH = max(dot(normal, halfVector), 0.0f);
    float NdotHSqr = NdotH * NdotH;
    
    float numerator = a2;
    float denomerator = (NdotHSqr * (a2 - 1.f) + 1.f);
...
{
    return F0 + (1 - F0) * pow(clamp(1 - cosTheta, 0.0f, 1.0f), 5);
}

float3 SchlickGGX(float NdotV, float roughness)
{
    float r = roughness + 1.0f;
    float k = (r * r) / 8.0f;
    
    float numerator = NdotV;
    float denomerator = NdotV * (1.0f - k) + k;
...
float4 PS(VS_OUTPUT input) : SV_TARGET
{
    ...
    float3 outgoingRadiance = 0.0f;
    {
        ...
        float fresnel = Schlick(max(dot(halfVector, -viewDirection), 0.0f), F0);
