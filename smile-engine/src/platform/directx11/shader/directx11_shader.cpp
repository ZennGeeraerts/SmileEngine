...
namespace Smile::Graphic
{
    ...
    void DirectX11Shader::Initalize( const std::string &assetFile, const std::string &techniqueName )
    {
        ...
        SM_ASSERT( m_pDirectX11Context, "DirectX11Shader::Initialize > Rendering context is not a DirectX 11 Graphics Context" );

        if ( !LoadEffect( m_pDirectX11Context->GetDevice(), assetFile ) )
        {
            SAFE_RELEASE( m_pEffect );
            SM_ASSERT( false, "DirectX11Shader::Initialize > Failed to load effect" );
        }
        ...
        if ( !m_pTechnique->IsValid() )
            SM_LOG_WARNING( "DirectX11Shader::Intitialize > Invalid technique" );
    ...
    bool DirectX11Shader::LoadEffect( ID3D11Device *pDevice, const std::string &assetFile )
    {
        ...
        result = D3DX11CompileEffectFromFile(
            assetFileWideStr.c_str(), nullptr, nullptr, shaderFlags, 0, pDevice, &m_pEffect, &pErrorBlob );
