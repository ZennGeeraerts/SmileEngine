...
namespace smile
{
    ...
    {
        if ( bSkinned )
            m_pShader = Shader::Create( "assets/shaders/PBR_Skinned.fx" );
        else
            m_pShader = Shader::Create( "assets/shaders/PBR.fx" );

        auto pEnvironmentMap = Texture2D::Create( "assets/textures/Sunol_Cubemap.dds" );
