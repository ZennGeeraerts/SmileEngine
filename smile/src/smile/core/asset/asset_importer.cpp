...
namespace smile::asset
{
    void AssetImporter::RegisterLoader( AssetLoader *pLoader )
    {
        ...
        SM_ASSERT_MSG( m_AssetLoaderMap.FindItem( assetType ) == m_AssetLoaderMap.end(), "Asset type collision" );
        m_AssetLoaderMap.Add( assetType, pLoader );

        for ( const fs::Path &extension : pLoader->GetExtensions() )
        {
            SM_ASSERT_MSG(
                m_AssetExtensionMap.FindItem( extension ) == m_AssetExtensionMap.end(), "Extension collision" );
    ...
    {
        if ( m_AssetLoaderMap.FindItem( metadata.Type ) == m_AssetLoaderMap.end() )
        {
            SM_LOG_ERROR(
    ...
    {
        if ( m_AssetExtensionMap.FindItem( extension ) == m_AssetExtensionMap.end() )
        {
