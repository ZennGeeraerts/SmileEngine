...
#include "Platform/DirectX11/DirectX11Diagnostics.h"
...
namespace smile
{
    ...
    DirectX11Framebuffer::DirectX11Framebuffer( const FramebufferData &framebufferData ) : m_Data{ framebufferData }
    {
        ...
        for ( const auto &fbTextureData : m_Data.m_Attachments.m_Attachments )
        {
            if ( !IsDepthFormat( fbTextureData.m_TextureFormat ) )
    ...
    void DirectX11Framebuffer::Invalidate()
    {
        ...
        if ( m_ColorAttachmentData.size() )
        {
            ...
            for ( uint32_t i{}; i < m_pColorAttachments.size(); ++i )
            {
                ...
                textureDesc.Format =
                    FramebufferTextureFormatToDirectXBaseType( m_ColorAttachmentData[i].m_TextureFormat );
                ...
                if ( m_ColorAttachmentData[i].m_bUseInShader )
                {
        ...
        if ( m_DepthAttachmentData.m_TextureFormat != FramebufferTextureFormat::None )
        {
            D3D11_TEXTURE2D_DESC depthStencilDesc{};
            depthStencilDesc.Width = m_Data.m_Width;
        ...
        m_Viewport.Width = static_cast< FLOAT >( m_Data.m_Width );
        m_Viewport.Height = static_cast< FLOAT >( m_Data.m_Height );
    ...
    void DirectX11Framebuffer::Unbind() const
    {
        ...
        m_pDirectX11Context->GetDeviceContext()->OMSetRenderTargets(
            1, &pRenderTargetView, m_pDirectX11Context->GetDepthStencilView() );
    ...
    {
        if ( ( width <= 0 ) || ( height <= 0 ) || ( width > m_MaxFramebufferSize ) ||
             ( height > m_MaxFramebufferSize ) )
        {
