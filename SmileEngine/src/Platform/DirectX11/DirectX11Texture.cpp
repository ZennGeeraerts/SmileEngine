...
namespace Smile
{
	...
	bool DirectX11Texture2D::LoadTexture(const std::string& filePath)
	{
		...
		result = DirectX::CreateTexture(m_pDirectX11Context->GetDevice(), image.GetImages(), image.GetImageCount(), image.GetMetadata(), &m_pTexture);
		if (FAILED(result))
		{
		...
		result = DirectX::CreateShaderResourceView(m_pDirectX11Context->GetDevice(), image.GetImages(), image.GetImageCount(), image.GetMetadata(), &m_pShaderResourceView);
		if (FAILED(result))
		{
