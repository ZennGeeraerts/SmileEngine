...
namespace Smile
{
	EditorCamera::EditorCamera(float fov, float aspectRatio, float nearPlane, float farPlane)
		...
		, m_FarPlane{ farPlane }
		, Camera(DirectX::XMMatrixPerspectiveFovLH(DirectX::XMConvertToRadians(fov), aspectRatio, nearPlane, farPlane))
	{
	...
	{
		if (Input::IsKeyPressed(Key::eAlt))
		{
			...
			DirectX::XMVECTOR deltaVec = DirectX::XMVectorScale((DirectX::XMVectorSubtract(mousePositionVec, initialMousePosVec)), 0.003f);
			m_InitialMousePosition = mousePosition;

			DirectX::XMFLOAT2 delta{};
			DirectX::XMStoreFloat2(&delta, deltaVec);

			if (Input::IsMouseButtonPressed(Mouse::eButtonMiddle))
				MousePan(delta);
			else if (Input::IsMouseButtonPressed(Mouse::eButtonLeft))
				MouseRotate(delta);
			else if (Input::IsMouseButtonPressed(Mouse::eButtonRight))
				MouseZoom(delta.y);
		}
	...
	void EditorCamera::UpdateView()
	{
		...
		DirectX::XMMATRIX transformMat = DirectX::XMMatrixMultiply(
	...
	void EditorCamera::UpdateProjection()
	{
		...
		DirectX::XMMATRIX projectionMatrixMat = DirectX::XMMatrixPerspectiveFovLH(DirectX::XMConvertToRadians(m_FOV), m_AspectRatio, m_NearPlane, m_FarPlane);
	...
	{
		DirectX::XMMATRIX viewProjectionMatrixMat = DirectX::XMLoadFloat4x4(&m_ViewMatrix) * DirectX::XMLoadFloat4x4(&m_ProjectionMatrix);
	...
	void EditorCamera::MousePan(const DirectX::XMFLOAT2& delta)
	{
		...
		focalPointVec = DirectX::XMVectorAdd(focalPointVec, DirectX::XMVectorScale(
			DirectX::XMVectorScale(
				DirectX::XMVectorScale(DirectX::XMVectorNegate(rightDirectionVec), delta.x),
				panSpeed.x), m_Distance)
		);

		focalPointVec = DirectX::XMVectorAdd(focalPointVec, DirectX::XMVectorScale(
			DirectX::XMVectorScale(
				DirectX::XMVectorScale(upDirectionVec, delta.y),
			panSpeed.y), m_Distance)
		);
	...
	DirectX::XMFLOAT3 EditorCamera::GetUpDirection() const
	{
		...
		DirectX::XMVECTOR rotationVec = DirectX::XMVector3Rotate(upVec, DirectX::XMQuaternionRotationRollPitchYaw(-m_Pitch, -m_Yaw, 0.f));
	...
	DirectX::XMFLOAT3 EditorCamera::GetRightDirection() const
	{
		...
		DirectX::XMVECTOR rotationVec = DirectX::XMVector3Rotate(rightDirection, DirectX::XMQuaternionRotationRollPitchYaw(-m_Pitch, -m_Yaw, 0.f));
		DirectX::XMStoreFloat3(&rotation, rotationVec);
		return rotation;
	}
	...
	DirectX::XMFLOAT3 EditorCamera::GetForwardDirection() const
	{
		...
		DirectX::XMVECTOR rotationVec = DirectX::XMVector3Rotate(forwardDirection, DirectX::XMQuaternionRotationRollPitchYaw(-m_Pitch, -m_Yaw, 0.f));
		DirectX::XMStoreFloat3(&rotation, rotationVec);
		return rotation;
	}
}
