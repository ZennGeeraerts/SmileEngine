...
namespace smile
{
    ...
    void Bone::OnUpdate( float animationTime )
    {
        ...
        DirectX::XMMATRIX localTransformMat = DirectX::XMMatrixTransformation( DirectX::XMVectorZero(),
    ...
    void Bone::InterpolateTranslation( DirectX::XMFLOAT3 &translation, float animationTime )
    {
        ...
        {
            translation = {
                m_Translations[0].m_Translation.x, m_Translations[0].m_Translation.y, m_Translations[0].m_Translation.z };
            return;
        }

        uint32_t p0Index = GetTranslationIndex( animationTime );
        uint32_t p1Index = p0Index + 1;
        float scaleFactor = GetScaleFactor( m_Translations[p0Index].m_Tick, m_Translations[p1Index].m_Tick, animationTime );

        DirectX::XMVECTOR finalTranslationVec =
            DirectX::XMVectorLerp( DirectX::XMLoadFloat3( &m_Translations[p0Index].m_Translation ),
                DirectX::XMLoadFloat3( &m_Translations[p1Index].m_Translation ),
                scaleFactor );
    ...
    uint32_t Bone::GetTranslationIndex( float animationTime )
    {
        ...
        {
            if ( animationTime <= m_Translations[static_cast< size_t >( i ) + 1].m_Tick )
                return i;
        }
    ...
    void Bone::InterpolateRotation( DirectX::XMFLOAT4 &rotation, float animationTime )
    {
        ...
        if ( m_RotationCount == 1 )
        {
            DirectX::XMVECTOR rotationVec =
                DirectX::XMQuaternionNormalize( DirectX::XMLoadFloat4( &m_Rotations[0].m_Rotation ) );
        ...
        uint32_t p1Index = p0Index + 1;
        float scaleFactor = GetScaleFactor( m_Rotations[p0Index].m_Tick, m_Rotations[p1Index].m_Tick, animationTime );

        DirectX::XMVECTOR finalRotationVec =
            DirectX::XMQuaternionSlerp( DirectX::XMLoadFloat4( &m_Rotations[p0Index].m_Rotation ),
                DirectX::XMLoadFloat4( &m_Rotations[p1Index].m_Rotation ),
                scaleFactor );
    ...
    uint32_t Bone::GetRotationIndex( float animationTime )
    {
        ...
        {
            if ( animationTime <= m_Rotations[static_cast< size_t >( i ) + 1].m_Tick )
                return i;
        }
    ...
    void Bone::InterpolateScale( DirectX::XMFLOAT3 &scale, float animationTime )
    {
        ...
        if ( m_ScaleCount == 1 )
        {
            scale = { m_Scales[0].m_Scale.x, m_Scales[0].m_Scale.y, m_Scales[0].m_Scale.z };
            return;
        }

        uint32_t p0Index = GetScaleIndex( animationTime );
        uint32_t p1Index = p0Index + 1;
        float scaleFactor = GetScaleFactor( m_Scales[p0Index].m_Tick, m_Scales[p1Index].m_Tick, animationTime );
        DirectX::XMVECTOR finalScaleVec = DirectX::XMVectorLerp( DirectX::XMLoadFloat3( &m_Scales[p0Index].m_Scale ),
            DirectX::XMLoadFloat3( &m_Scales[p1Index].m_Scale ),
            scaleFactor );
    ...
    uint32_t Bone::GetScaleIndex( float animationTime )
    {
        ...
        {
            if ( animationTime <= m_Scales[static_cast< size_t >( i ) + 1].m_Tick )
                return i;
        }
