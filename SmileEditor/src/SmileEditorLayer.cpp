...
namespace Smile
{
	...
	void SmileEditorLayer::OnImGuiRender()
	{
		...
		if (selectedEntity && (m_GizmoType != GizmoType::eNone) && (m_SceneState == SceneState::eEdit))
		{
			...
			// Snapping
			bool bSnapping = Input::IsKeyPressed(Key::eLeftControl);
	...
	bool SmileEditorLayer::OnKeyPressed(KeyPressedEvent& e)
	{
		...
		bool bControlPressed = Input::IsKeyPressed(Key::eLeftControl) || Input::IsKeyPressed(Key::eRightControl);
		bool bShiftPressed = Input::IsKeyPressed(Key::eLeftShift) || Input::IsKeyPressed(Key::eRightShift);
	...
	void SmileEditorLayer::OpenScene(const std::filesystem::path& filePath)
	{
		...
		if (sceneSerializer.Deserialize(filePath.string()))
		{
			...
			m_pEditorScene->OnViewportResize(static_cast<uint32_t>(m_ViewportSize.x), static_cast<uint32_t>(m_ViewportSize.y));
	...
	{
		if (m_SceneState == SceneState::ePlay)
			OnSceneStop();
